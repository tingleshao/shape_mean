...
for i = 2:row-1,
    for j = 2:col-1,
        if binary_im(i-1,j) > 0  && binary_im(i+1,j) >0 && binary_im(i,j-1) >0 && binary_im(i,j+1) > 0
            continue
        elseif  binary_im(i-1,j) == 0  && binary_im(i+1,j) == 0 && binary_im(i,j-1) ==0 && binary_im(i,j+1) == 0
            continue
        else
            contour_im(i,j) = 1;
        end
